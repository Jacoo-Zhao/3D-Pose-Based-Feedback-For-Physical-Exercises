'''
This is the code from SGN[https://github.com/microsoft/SGN]
...
Note: We have made some changes to the code that are applicable to our project. You can find comments.
'''
...
# root_path = '../data/ntu/SGN_output'
# raw_data_file = osp.join(root_path, 'raw_data', 'raw_skes_data.pkl')
# save_path = osp.join(root_path, 'denoised_data_with_actor_one')

raw_data_file = '3D-Motion-Correction/Work_on_related_works/data/ntu/SGN_output/raw_data/raw_skes_data.pkl'
save_path = '3D-Motion-Correction/Work_on_related_works/data/ntu/SGN_output/denoised_data_for_first_actor_v2'
...
def get_raw_denoised_data(count=0,len_bodies_data=[]):
    """
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        len_bodies_data.append(len(bodies_data['data']))

        # for 3D-Pose-Correction Project [EPFL-CVlab] 
        # delete the data of the second/(third)/(forth) actor  
        while(len(bodies_data['data'])!=1):
            bodies_data['data'].popitem() 
    ...
    return count, len_bodies_data

if __name__ == '__main__':
    count, len_bodies_data = get_raw_denoised_data()
