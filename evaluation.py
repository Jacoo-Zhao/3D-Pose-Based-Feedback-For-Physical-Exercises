...
from PoseCorrection.model import GCN_corr_ori, GCN_class, GCN_class_22Spring
from PoseCorrection.opt1 import Options
from PoseCorrection.utils1 import *
from PoseCorrection.model_combined_v1 import *
from PoseCorrection.model_combined_v2 import *
from utils import display_poses
...
def main_eval(time, savepath, model_combined, options, separated=False, model_corr_path='', model_class_path='', model_version='', model_combined_path=''):
        ...
        for i, (batch_id, inputs) in enumerate(test_loader):
            ...
            if separated:
                ...
                if model_version=='CCF22S':
                    ...
                    _, pred_out = torch.max(model(outputs, labels, False)[2].data, 1)
                    test_l_class, te_acc, sum, cmt = test_class_v4(test_loader, model, is_cuda=is_cuda, level=1, Use_label=False)
                    pd.DataFrame(cmt.numpy()).to_csv(savepath+'CMT.csv')
                    # #plotting
    ...
    loss_matrix.to_csv(savepath, mode='a', float_format='%6f')
...
if __name__ == "__main__":
    ...
    # model_combined_path = 'Results-22Spring/Combined_v2/models/2022-06-14-16-34.pt'
    ...
    main_eval(time, savepath,  model_version='CCF22S', model_combined= model, model_combined_path=opt.model_dir, options=opt)
